...
def run_evaluation(data_dir, result_folder, model_weights_path, num_classes, batch_size, seed, model_type, output_cam=False, cam_method='gradcam'):
    ...
    # 修改：移除"IMC"类别，仅保留3个类别
    categories = ['columnar', 'equiax', 'background']
    class_to_label = {"equiax": 0, "columnar": 1, "background": 2}
    overall_y_true = []
    overall_y_pred = []
    ...
    for category in categories:
        ...
        if not os.path.exists(category_path):
            print(f"类别路径 {category_path} 不存在，跳过...")
            continue
        ...
        for subfolder in subfolders:
            ...
            for images, labels_batch, paths_batch in tqdm(data_loader_test, desc='Evaluating'):
                ...
                for i in range(len(labels_batch)):
                    # 安全获取概率值
                    prob0 = probabilities[i, 0].item() if probabilities.shape[1] > 0 else 0.0
                    prob1 = probabilities[i, 1].item() if probabilities.shape[1] > 1 else 0.0
                    prob2 = probabilities[i, 2].item() if probabilities.shape[1] > 2 else 0.0
            ...
            try:
                ...
                plt.plot(x_indices, image_probabilities_class0, marker='o', label='Equiaxed')
                plt.plot(x_indices, image_probabilities_class1, marker='^', label='Columnar')
                plt.plot(x_indices, image_probabilities_class2, marker='s', label='Background')
                
                all_probs = (image_probabilities_class0 + image_probabilities_class1 + image_probabilities_class2)
                ymin = min(all_probs) - 0.01 if all_probs else 0
                ymax = max(all_probs) + 0.01 if all_probs else 1
                plt.ylim(ymin, ymax)
                plt.xlabel('图像索引')
                plt.ylabel('概率')
                plt.title('各图像类别概率')
                ...
                print(f"生成概率曲线图时出错: {e}")
    
    if not overall_y_true or not overall_y_pred:
        print("没有收集到任何真实标签或预测标签，无法计算准确率。")
        return 0.0
    ...
    try:
        ...
        print(f"生成总体混淆矩阵时出错: {e}")
    
    # 调用原有功能（动画生成、每视频准确率统计、整体平衡准确率计算）
    # 注意：确保utils_analysis函数能正确处理3类数据
    try:
...
def main():
    ...
    cam_method = 'original_cam'    # 可选 'gradcam', 'gradcam++', 'scorecam', 'original_cam'
